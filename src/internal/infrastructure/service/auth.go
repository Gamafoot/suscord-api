...
func (s *authService) Login(ctx context.Context, input *entity.LoginOrCreateInput) (string, error) {
	...
	user, err := s.storage.Database().User().GetByUsername(ctx, input.Username)
	if err != nil {
		...
		err = s.storage.Database().User().Create(ctx, &entity.User{
...
func (s *authService) createSession(ctx context.Context, userID uint) (string, error) {
	...
	_, err := s.storage.Database().Session().GetByUserID(ctx, userID)
	if err != nil {
		if pkgErrors.Is(err, domainErrors.ErrRecordNotFound) {
			uuid, err = s.storage.Database().Session().Create(ctx, userID)
			if err != nil {
				return "", err
			}
			return uuid, nil
		...
		uuid, err = s.storage.Database().Session().Update(ctx, userID)
		if err != nil {
			return "", err
		}
	}
